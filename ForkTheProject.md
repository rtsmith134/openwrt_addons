...
2. execute `gen_new_usign_key.sh` and `gen_new_gpg_key.sh` generate usign&gpg key for yourself. This is based on https://github.com/fantastic-packages/packages/blob/master/ForkTheProject.md. These two scripts will create files in the following locations.
   + `./keys/usign/*.pub`
   + `./keys/usign/*.sec`
   + `./keys/gpg/*.finger`
   + `./keys/gpg/*.pub`
   + `./keys/gpg/*.rev`
   + `./keys/gpg/*.sec`
3. goto `https://github.com/<your_fork>/settings/secrets/actions`, and add your secret keys that you created in step 2.
...
4. goto `https://github.com/<your_fork>/settings/variables/actions`, and add your public keys
   ...
   + `VERIFY_KEY_ID`: <your key>53FF2B6672243D28
